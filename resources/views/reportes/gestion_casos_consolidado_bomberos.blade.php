...
<h1>Reporte Estadistico por Bomberos: </h1>
<h2>Desde Fecha: {{$feini}} <br> Hasta Fecha: {{$fefin}}</h2>
  ...
  @foreach($datos as $d)
  <tr>
    <td id="d" width ='30%'>Cedula:<strong>{{number_format($d->cedbombero,0,',','.')}}</strong> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Rango: <strong>{{$d->rango}}</strong><br> Nombre: <strong>{{$d->nombombero}}</strong> <br> Apellido: <strong>{{$d->apebombero}}</strong></td>
    <td>{{$d->casos}}</td> 
